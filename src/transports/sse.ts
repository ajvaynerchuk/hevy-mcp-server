 ...
 */
export function createSSETransport(server: Server, config: SSETransportConfig): express.Application {
    const app = express();
    ...
    app.post(config.ssePath, async (req: Request, res: Response) => {
        try {
            ...
            if (!sessionId) {
                ...
                res.status(400).json({
                    error: 'Missing sessionId in Mcp-Session-Id header or sessionId query parameter',
                });
                return;
            }
 ...
 */
export async function initializeSSETransport(server: Server, config: SSETransportConfig): Promise<void> {
